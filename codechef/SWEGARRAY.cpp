...
/*--------finding inverse modulo 10^9+7------*/
int changedval(int val, int l){
    if(val==l)return val+1;
    return l;
}
int minvals[100005];
int main(){
    ...
    while(t--){
        ...
        if(l==r){
           // printf("-1\n");
            ...
            if(n==1){
                printf("%d\n",changedval(allnum[0],l));
            }//apps.facebook.com/pool-live/?fb_source=bookmark&ref=bookmarks&count=0&fb_bmpos=_0
            else{
                printf("%d ",l);
                for(int i=0;i<n-1;++i){
                    printf("%d",allnum[i]);
