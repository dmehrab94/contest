...
long long int Marray[100006];
long long int invmods[100006];
int main(){
    int t;
    scanf("%d",&t);
    for(int i=1;i<100006;++i){
        long long int a=(long long int)i;
        invmods[i]=findMMI_fermat(a,mod);
    }
    while(t--){
        long long int n,x,M;
        ...
        long long int val=0;
        while(x>=0){
            val=val+((arr[x]%mod)*(Marray[j]))%mod;
            val%=mod;
            j++;
            x--;
        }
