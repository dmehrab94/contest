...
int main(){
        ...
        int start2=2;
        int nowstart2=0;
        map<int,int>nowstart2map;
        for(int i=1;i<=comm;++i){
            if(start2%6==0 && start2<=min3){
                start2+=2;
            }
            nowstart2=start2;
            nowstart2map[nowstart2]=1;
            start2+=2;
        }
        cout<<min3<<" "<<nowstart2<<"\n";
        int min2=comm*2;
        int start3=3;
        int nowstart3=0;
        map<int,int>nowstart3map;
        for(int i=1;i<=comm;++i){
            if(start3%6==0 && start3<=min2){
                start3+=3;

            }
            nowstart3=start3;
            nowstart3map[nowstart3]=1;
            start3+=3;
        }
        cout<<min2<<" "<<nowstart3<<"\n";
        if(n>m){
            for(int i=comm+1;i<=n;++i){
                nowstart2+=2;
                if(nowstart2%6==0 && nowstart2<=min3)nowstart2+=2;
                min2+=2;
                while(!(nowstart3map.find(min2)==nowstart3map.end())){
                    min2+=2;
                }
            }
            cout<<nowstart2<<" "<<min2<<"\n";
            //cout<<nowstart2<<" "<<min2<<"\n";
            cout<<min(max(min2,nowstart3),max(min3,nowstart2))<<"\n";
        }
        else{

            for(int i=comm+1;i<=m;++i){
                nowstart3+=3;
                if(nowstart3%6==0 && nowstart3<=min2)nowstart3+=3;
                min3+=3;
                while(!(nowstart2map.find(min3)==nowstart2map.end())){
                    min3+=3;
                }
            }
            cout<<min(max(min2,nowstart3),max(min3,nowstart2))<<"\n";
