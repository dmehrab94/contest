...
int main(){
    ...
    for(int i=1;i<=movie;++i){
        scanf("%d",&movieaudio[i]);
    }
    for(int i=1;i<=movie;++i){
        scanf("%d",&moviesub[i]);
    }
    vector<myitem>pleasure;
    for(int i=1;i<=movie;++i){
        int aud=movieaudio[i];
        int sub=moviesub[i];
        int please=0;
        if(lang.find(aud)!=lang.end())please=lang[aud];
        int almost=0;
        if(lang.find(sub)!=lang.end())almost=lang[sub];
        myitem m(almost,please,i);
        pleasure.push_back(m);
    }
    sort(pleasure.begin(),pleasure.end(),mycompare);
    cout<<pleasure[0].movie<<"\n";
}
